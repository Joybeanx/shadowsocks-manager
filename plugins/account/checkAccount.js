...
const setKcptun = async (port, kcptunPort) => {
    const server = await serverManager.list();
    const account = await knex('account_plugin').select();
    const portExist = account.some(f => f.port === port);
    if (!portExist) {
        return Promise.reject(`shadowsocks port[${port }] doesn't not exists`);
    }
    server.forEach(s => {
        messages.push([{
            command: 'setKcptun',
            port: port + s.shift,
            kcptunPort: kcptunPort,
        }, {
            host: s.host,
            port: s.port,
            password: s.password,
...
const checkServer = async () => {
    ...
    let isMultiServerFlow = false;
    try {
        isMultiServerFlow = await knex('webguiSetting').select().where({key: 'account'})
            .then(success => {
                if (!success.length) {
                    return Promise.reject('settings not found');
                }
